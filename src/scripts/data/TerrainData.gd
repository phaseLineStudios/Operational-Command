...
@export var grid_start_y: int = 100:
	set = _set_grid_y
...
@export var contour_interval_m: int = 10:
	set = _set_contour_interval_m
## Elevation image (R channel = meters; 16F or 32F preferred).
@export var elevation: Image = Image.create(64, 64, false, Image.FORMAT_RF):
	set = _set_elev
