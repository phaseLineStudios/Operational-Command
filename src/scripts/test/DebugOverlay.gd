...
func _draw_unit_glyphs(su: ScenarioUnit, _idx: int) -> void:
	...
	if show_bars and su.unit:
		...
		var label := (su.callsign if su.callsign != "" else (su.id if su.id != "" else (su.unit.id if su.unit else "")))
		if label != "":
...
func _draw_text_panel(d: Dictionary) -> void:
	...
	lines.append("ATK base=%.1f → %.1f | DEF base=%.1f → %.1f" % [
		float(d.get("base_attack", 0.0)), float(d.get("attackpower", 0.0)),
		float(d.get("base_defense", 0.0)), float(d.get("defensepower", 0.0))
	])
	if d.has("attacker") and d.has("defender"):
		lines.append("%s S%.0f/M%.2f  vs  %s S%.0f/M%.2f" % [
			String(d.attacker.cs), float(d.attacker.strength), float(d.attacker.morale),
	...
	var tl := Vector2(
		clamp(anchor.x, 0.0, size.x - panel_size.x),
		clamp(anchor.y, 0.0, size.y - panel_size.y)
	)
