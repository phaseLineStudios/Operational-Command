...
## Scenario JSON file
@export_file("*.json ; Scenario") var scenario_data_json: String = "res://scripts/test/combat_test_scenario.json"
...
func _ready() -> void:
	if renderer == null or renderer.data == null:
		push_warning("Setup: TerrainRender or TerrainData missing."); return
	if renderer.path_grid == null:
		push_warning("Setup: Assign PathGrid to TerrainRender.path_grid."); return
	...
	if _scenario == null or _scenario.units.is_empty():
		push_warning("Setup: Scenario invalid or has no units."); return

	_su_a = _find_su(_scenario.units, unit_a_id)
	_su_b = _find_su(_scenario.units, unit_b_id)
	if _su_a == null or _su_b == null:
		push_warning("Setup: Could not find both units in scenario."); return
...
func _process(dt: float) -> void:
	if renderer and renderer.path_grid:
		if _su_a: _su_a.tick(dt, renderer.path_grid)
		if _su_b: _su_b.tick(dt, renderer.path_grid)
	if input_overlay: input_overlay.queue_redraw()

func _input(e: InputEvent) -> void:
	if not (e is InputEventMouseButton and e.pressed): return
	if renderer == null or _scenario == null: return
	var me := e as InputEventMouseButton
	var pos := me.position
	if not renderer.is_inside_terrain(pos):
		print("Outside terrain:", pos); return
...
func _move_su_to(su: ScenarioUnit, dest_m: Vector2) -> void:
	if su == null or renderer.path_grid == null: return
...
func _load_scenario_json(path: String) -> Dictionary:
	if path == "" or not FileAccess.file_exists(path):
		push_warning("Scenario JSON not found: " + path); return {}
...
func _find_su(list: Array[ScenarioUnit], key: String) -> ScenarioUnit:
	for su in list:
		if su == null: continue
		if su.id == key: return su
		if su.unit and su.unit.id == key: return su
	return null
