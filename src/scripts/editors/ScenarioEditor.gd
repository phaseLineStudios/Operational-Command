...
func _on_overlay_gui_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
		...
		if draglink.linking: draglink.update_link(ctx, event.position)
		elif draglink.dragging: draglink.update_drag(ctx, event.position)
	...
	if event is InputEventMouseButton:
		...
		if event.button_index == MOUSE_BUTTON_LEFT:
			...
			if pick.is_empty(): selection.clear_selection(ctx)
			else:
...
func _delete_pick(pick: Dictionary) -> void:
	match StringName(pick.get("type","")):
		...
		&"slot": _delete_slot(int(pick["index"]))
		&"task": _delete_task(int(pick["index"]))
...
func _on_attributemenu_pressed(id: int):
	match id:
		0:
			if ctx.data: new_scenario_dialog.show_dialog(true, ctx.data)
			else: new_scenario_dialog.show_dialog(true)
		1:
	...
	if ctx.data.units:
		for su in ctx.data.units:
			if su and su.affiliation == affiliation:
				...
				if not cs.is_empty(): used[cs] = true
