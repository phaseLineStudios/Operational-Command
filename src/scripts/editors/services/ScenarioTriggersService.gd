...
func build_palette(ctx: ScenarioEditorContext) -> void:
	...
	var img := proto.icon.get_image(); img.resize(24,24,Image.INTERPOLATE_LANCZOS)
...
func try_sync_link(ctx: ScenarioEditorContext, a: Dictionary, b: Dictionary) -> void:
	...
	if ctx.data == null or ctx.data.triggers == null: return
	if trig_idx < 0 or trig_idx >= ctx.data.triggers.size(): return
	var trig: ScenarioTrigger = ctx.data.triggers[trig_idx]
	if trig == null: return
	if unit_idx >= 0:
		if trig.synced_units == null: trig.synced_units = []
		if not trig.synced_units.has(unit_idx): trig.synced_units.append(unit_idx)
	if task_idx >= 0:
		if trig.synced_tasks == null: trig.synced_tasks = []
		if not trig.synced_tasks.has(task_idx): trig.synced_tasks.append(task_idx)
	ctx.request_overlay_redraw()
	ctx.request_scene_tree_rebuild()
