...
func trace_los(
	a: Vector2, d: Vector2, renderer: Variant, terrain: Variant, cfg: TerrainEffectsConfig
) -> Dictionary:
	var res := TerrainEffects._compute_los_and_atten(a, d, renderer, terrain, cfg)
	return {
		"blocked": res.blocked,
		"atten_integral": float(res.atten_integral),
		"range_m": a.distance_to(d)
	}


## Spotting multiplier from conceal + weather
func compute_spotting_mul(
