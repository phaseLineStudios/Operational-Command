...
Priority: target unit (when `prefer_target`) -> grid position
...
`unit` Subject unit (for direction-based movement).
`order` Order dictionary.
`prefer_target` If `true`, prefer unit target for ATTACK.
